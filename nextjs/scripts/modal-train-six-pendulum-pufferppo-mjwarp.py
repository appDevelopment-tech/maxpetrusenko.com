...
def build_contract(total_timesteps: int = 10_000_000) -> dict:
    ...
    return {
        ...
        "gpuKernelProgress": {
            ...
            "envIntegration": "SixPendulumMJWarpPufferEnv now uses the Warp score kernel after reset/step and reports scoreBackend=warp-score-kernel in env-driver artifacts.",
            "notCovered": "Reset, ctrl writes, truncation, held-step accumulation, potential-delta reward, and Puffer rollout integration.",
        },
        "gpuKernelBlocker": {
            "currentEnv": "scripts/six_pendulum_mjwarp_env.py",
            "problem": "Current env still uses CPU reset sampling through d.qpos.numpy()/d.qvel.numpy(), numpy action-to-ctrl writes, CPU held-step/potential-delta bookkeeping, and CPU metric copies for the PufferEnv interface.",
