...
def build_contract(total_timesteps: int = 10_000_000) -> dict:
    ...
    return {
        ...
        "gpuKernelProgress": {
            ...
            "envIntegration": "SixPendulumMJWarpPufferEnv now uses Warp kernels for action scaling, ctrl writes, score/reward/observation, and cart terminal; env-driver artifacts report scoreBackend=warp-score-kernel.",
            "notCovered": "Reset sampling, truncation, held-step accumulation, potential-delta reward, and Puffer rollout integration.",
        },
        "gpuKernelBlocker": {
            "currentEnv": "scripts/six_pendulum_mjwarp_env.py",
            "problem": "Current env still uses CPU reset sampling through d.qpos.numpy()/d.qvel.numpy(), CPU held-step/potential-delta bookkeeping, and CPU metric copies for the PufferEnv interface.",
            ...
            "requiredBeforeRealSweep": [
                ...
                "Move truncation, held-step, max-held-step, and potential-delta reward math GPU-side.",
