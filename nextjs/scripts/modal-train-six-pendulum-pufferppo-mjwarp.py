...
def build_contract(total_timesteps: int = 10_000_000) -> dict:
    ...
    return {
        ...
        "gpuKernelProgress": {
            ...
            "envIntegration": "SixPendulumMJWarpPufferEnv now uses Warp kernels for action scaling, ctrl writes, score/reward/observation, cart terminal, truncation, held/max-held accumulation, and potential-delta reward; env-driver artifacts report scoreBackend=warp-score-kernel and rolloutBackend=warp-post-step-kernel.",
            "notCovered": "Reset sampling, done-world reset integration, CPU metric copies for the current PufferEnv interface, and Puffer rollout integration.",
        },
        "gpuKernelBlocker": {
            "currentEnv": "scripts/six_pendulum_mjwarp_env.py",
            "problem": "Current env still uses CPU reset sampling through d.qpos.numpy()/d.qvel.numpy(), done-world reset sync, and CPU metric copies for the current PufferEnv interface.",
            ...
            "requiredBeforeRealSweep": [
                "Move reset sampling and done-world reset state sync GPU-side.",
