...
DEFAULT_OUTPUT = DEFAULT_CHECKPOINT_DIR / "puffer-mjwarp-device-ppo-link1-awac-replay-f160-20260611.json"
DEFAULT_CHECKPOINT = DEFAULT_CHECKPOINT_DIR / "puffer-mjwarp-device-ppo-link1-awac-replay-f160-20260611.pt"


def flatten_window_transitions(windows: list[dict]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict]:
    obs_parts = []
    action_parts = []
    reward_parts = []
    next_obs_parts = []
    ...
    for window in windows:
        observations = window["observations"].astype(np.float32, copy=False)
        actions = window["actions"].astype(np.float32, copy=False)
        if observations.shape[0] < 2:
            continue
        rewards = hold_quality(observations[1:])
        count = observations.shape[0] - 1
        obs_parts.append(observations[:-1])
        action_parts.append(actions[:-1])
        reward_parts.append(rewards)
        next_obs_parts.append(observations[1:])
        phase_weight = np.full((count,), float(window["weight"]), dtype=np.float32)
        weight_parts.append(phase_weight)
    ...
    if not obs_parts:
        ...
        return empty_obs, empty, empty, empty_obs, empty, {"transitionCount": 0, "phaseTransitionCounts": phase_counts}
    ...
    weights = np.concatenate(weight_parts, axis=0).astype(np.float32, copy=False)
    weights = weights / max(float(np.mean(weights)), 1e-6)
    return obs, actions, rewards, next_obs, weights, {
        "transitionCount": int(obs.shape[0]),
        "phaseTransitionCounts": phase_counts,
        "rewardMean": float(np.mean(rewards)),
        "rewardMax": float(np.max(rewards)),
        "phaseWeightMean": float(np.mean(weights)),
        "phaseWeightMax": float(np.max(weights)),
    }


class TwinCriticBuilder:
    @staticmethod
    def build(obs_dim: int, hidden_dim: int):
        import torch

        class TwinCritic(torch.nn.Module):
            def __init__(self):
                super().__init__()
                input_dim = int(obs_dim) + 1
            ...
            def forward(self, obs, action):
                ...
                return self.q1(x).reshape(-1), self.q2(x).reshape(-1)

        return TwinCritic()


def train_awac_replay(policy, old_policy, transitions, anchor_obs: np.ndarray, args: argparse.Namespace, hidden_dim: int):
    import torch
    ...
    for epoch in range(int(args.critic_epochs)):
        ...
        with torch.no_grad():
            ...
            next_action, _, _, _ = policy(batch_next_obs, next_hidden, deterministic=True)
            target_q1, target_q2 = target_critic(batch_next_obs, next_action)
            target_q = batch_rewards + float(args.gamma) * torch.minimum(target_q1, target_q2)
        q1, q2 = critic(batch_obs, batch_actions)
        critic_loss = ((q1 - target_q).pow(2) + (q2 - target_q).pow(2)) * batch_weights
        critic_loss = critic_loss.mean()
        critic_optimizer.zero_grad(set_to_none=True)
        critic_loss.backward()
        grad_norm = float(torch.nn.utils.clip_grad_norm_(critic.parameters(), float(args.grad_clip)).detach())
    ...
    for epoch in range(int(args.actor_epochs)):
        ...
        with torch.no_grad():
            data_q1, data_q2 = critic(batch_obs, batch_actions)
            data_q = torch.minimum(data_q1, data_q2)
            policy_action, _, _, _ = policy(batch_obs, hidden, deterministic=True)
            policy_q1, policy_q2 = critic(batch_obs, policy_action)
            baseline_q = torch.minimum(policy_q1, policy_q2)
            advantage = data_q - baseline_q
            awac_weight = torch.exp(advantage / max(float(args.advantage_beta), 1e-6))
            awac_weight = torch.clamp(awac_weight, min=float(args.min_weight), max=float(args.max_weight))
            awac_weight = awac_weight * batch_weights
        actor_loss = -(awac_weight * logprob).mean() - float(args.entropy_coef) * entropy.mean()
            ...
            actor_losses.append(
                {
                    ...
                    "weightMean": float(awac_weight.mean()),
                    "weightMax": float(awac_weight.max()),
                    "gradNorm": grad_norm,
...
def run_awac_replay(mjcf_xml: str, args: argparse.Namespace) -> dict:
    import torch
    ...
    if transitions[0].shape[0] == 0:
        awac = {"stats": transitions[-1], "criticLosses": [], "actorLosses": []}
        after_eval = before_eval
        status = "no-awac-transitions"
    else:
        awac = train_awac_replay(policy, old_policy, transitions, anchor_obs, args, hidden_dim)
    ...
    return {
        "schema": "six-pendulum-mjwarp-awac-replay-v1",
        "status": status,
        "algorithm": "phase-balanced-parameterized-teacher-dagger-awac-twin-critic-replay",
        "links": links,
        ...
        "awac": {
            ...
            "targetTau": float(args.target_tau),
            ...
            "criticLosses": awac["criticLosses"],
            "actorLosses": awac["actorLosses"],
        },
        ...
        "notes": [
            "This is the first local critic-guided replay lane after BC, DAgger, sequence BC, and shallow AWR failed.",
            "Twin critics train on phase-balanced teacher/learner transitions; the actor uses AWAC-style advantage-weighted log-prob updates.",
...
def main():
    parser = argparse.ArgumentParser(description="AWAC-style critic replay repair for MJWarp one-link exact down-start.")
    ...
    result = run_awac_replay(mjcf_path.read_text(), args)
