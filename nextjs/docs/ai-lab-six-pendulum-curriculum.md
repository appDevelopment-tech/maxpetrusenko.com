...
- Torch-policy bridge result on 2026-06-10: the rollout converts current Warp observations with `wp.to_torch(runner.obs_wp)`, runs a tiny Torch policy, converts normalized policy output back with `wp.from_torch(action_torch)`, and writes ctrl through `warp-policy-action-vector-kernel` with `torchPolicyCpuActionWritesPerStep=0`. The policy has `2241` parameters and is untrained, so it proves tensor plumbing only and does not count toward solve.
...
- Interpretation: reset sampling/writes, horizon sampling, action scaling, action-tensor ctrl writes, Torch/Warp policy-action interop, reward, observation, strict score, cart terminal, truncation, held/max-held accumulation, potential-delta reward, and fixed-shape rollout-buffer writes are now reusable. A standalone MJWarp rollout can keep metrics device-side per step, and a Torch policy can consume current Warp observations without NumPy action writes. The remaining Yacine-speed blocker is attaching this to PufferPPO/MinGRU recurrent state and GPU stream/capture discipline without falling back to the NumPy-returning PufferEnv step interface.
...
- Current blockers: Modal GPU execution is blocked by the workspace spend limit, and the current Puffer-facing env still returns NumPy observations/rewards/dones each step. The standalone device-rollout smoke proves the no-per-step-CPU-metric path; the action-buffer smoke proves a no-per-step-host-action-write interface; the Torch-policy smoke proves `wp.to_torch`/`wp.from_torch` policy-action plumbing. The real PufferPPO/MJWarp path must attach rollout tensors, normalized actions/logprobs, recurrent state, and GPU capture to Puffer before claiming Yacine-like speed.
