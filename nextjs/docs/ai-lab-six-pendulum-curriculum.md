...
- Interpretation: reset sampling/writes, horizon sampling, action scaling, action-tensor ctrl writes, Torch/Warp recurrent actor-critic interop, reward, observation, strict score, cart terminal, truncation, held/max-held accumulation, potential-delta reward, and fixed-shape PPO rollout-buffer writes are now reusable. A standalone MJWarp rollout can keep metrics device-side per step, and a recurrent Torch policy can consume current Warp observations without NumPy action writes. The remaining Yacine-speed blocker is making PufferPPO own these buffers, run minibatch updates, and obey GPU stream/capture discipline without falling back to the NumPy-returning PufferEnv step interface.
