...
- PPO-update result on 2026-06-10: one local CPU minibatch update consumed the fixed recurrent buffers, recomputed logprobs/values over the stored sequence, backpropagated, and changed parameters (`parameterDeltaL2=0.04349`, `gradNorm=0.04982`). This proves buffer-to-update plumbing only; it is not a PufferPPO sweep and not a learned solve.
...
- Interpretation: reset sampling/writes, horizon sampling, action scaling, action-tensor ctrl writes, Torch/Warp recurrent actor-critic interop, reward, observation, strict score, cart terminal, truncation, held/max-held accumulation, potential-delta reward, fixed-shape PPO rollout-buffer writes, and one PPO minibatch update are now reusable. A standalone MJWarp rollout can keep metrics device-side per step, and a recurrent Torch policy can consume current Warp observations without NumPy action writes. The remaining Yacine-speed blocker is replacing the local smoke update with real PufferPPO/MinGRU ownership of this path, multi-update sweeps, and GPU stream/capture discipline.
